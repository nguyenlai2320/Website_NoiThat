...
function loginFromSocialCallBack($socialUser) {
    ...
    $result = mysqli_query($con, "Select `id`,`username`,`email`,`fullname` from `dangnhap_fb` WHERE `email` ='" . $socialUser['email'] . "'");
    if ($result->num_rows == 0) {
        $result = mysqli_query($con, "INSERT INTO `dangnhap_fb` (`fullname`,`email`, `status`, `created_time`, `last_updated`) VALUES ('" . $socialUser['name'] . "', '" . $socialUser['email'] . "', 1, " . time() . ", '" . time() . "');");
        if (!$result) {
